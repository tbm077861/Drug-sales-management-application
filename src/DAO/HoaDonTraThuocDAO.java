...
public class HoaDonTraThuocDAO {
	public List<Object[]> loadDataToTable() {
		List<Object[]> data = new ArrayList<>();
		String sql = "SELECT maPTT, maNV, maKH, ngayTra, lyDoTra FROM PhieuTraThuoc";
		...
		try (Connection conn = ConnectDB.getConnection("DB_QuanLyNhaThuoc");
			...
			while (rs.next()) {
				...
				Object[] row = { rs.getString("maPTT"), rs.getString("maNV"), rs.getString("maKH"), 
	...
	public List<Object[]> searchHoaDon(String maHoaDon, String maNV, String maKH, String ngayTra) {
		List<Object[]> data = new ArrayList<>();
		StringBuilder sql = new StringBuilder("SELECT * FROM PhieuTraThuoc WHERE 1=1");
		...
		try (Connection conn = ConnectDB.getConnection("DB_QuanLyNhaThuoc");
			...
			while (rs.next()) {
				...
				Object[] row = { rs.getString("maPTT"), rs.getString("maNV"), rs.getString("maKH"), ngay, lyDoTra };
				data.add(row);
			}
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public List<Object[]> getAllHoaDon() {
		List<Object[]> data = new ArrayList<>();
		String sql = "SELECT * FROM PhieuTraThuoc";
