...
public class Dialog_InPhieuNhapThuoc extends JDialog {
	...
	public Dialog_InPhieuNhapThuoc(JFrame parent, String maHoaDon, String maNV, String maNCC, String ngayNhap,
			String trangThaiStr, String phuongThucThanhToanStr, java.util.List<Object[]> chiTietList) {
		super(parent, "Phiếu Nhập Thuốc - " + maHoaDon, true);
		...
		// Tiêu đề hóa đơn
		JLabel lblTieuDe = new JLabel("HÓA ĐƠN NHẬP THUỐC");
		...
		panelThongTin.setBounds(10, 173, 745, 143);
		...
		JLabel lblNgayNhap = new JLabel("Ngày Lập:");
		lblNgayNhap.setFont(new Font("Segoe UI", Font.PLAIN, 14));
		panelThongTin.add(lblNgayNhap);

		JLabel txtNgayNhap = new JLabel(ngayNhap);
		txtNgayNhap.setFont(new Font("Segoe UI", Font.PLAIN, 14));
		panelThongTin.add(txtNgayNhap);
		...
		// Sự kiện nút In
		btnInHoaDon.addActionListener(e -> btnInPhieuNhapThuocActionPerformed());
...
//		}
	...
	private void btnInPhieuNhapThuocActionPerformed() {
		PrinterJob printerJob = PrinterJob.getPrinterJob();
        printerJob.setJobName("Hóa Đơn Nhập Thuốc");
        ...
        if (printerJob.printDialog()) {
            try {
                printerJob.print();
                JOptionPane.showMessageDialog(this, "In phiếu nhập thuốc thành công!", "Thông Báo", JOptionPane.INFORMATION_MESSAGE);
                isPrinting = true;
                dispose();
            } catch (PrinterException ex) {
                JOptionPane.showMessageDialog(this, "Lỗi khi in phiếu nhập thuốc: " + ex.getMessage(), "Lỗi In", JOptionPane.ERROR_MESSAGE);
                isPrinting = false;
                ex.printStackTrace();
            }
		} else {
