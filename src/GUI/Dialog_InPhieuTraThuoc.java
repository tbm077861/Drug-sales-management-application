...
public class Dialog_InPhieuTraThuoc extends JDialog {
	...
	public Dialog_InPhieuTraThuoc(JFrame parent, String maHoaDon, String maNV, String maKH, String ngayTra,
			String lyDoTra, java.util.List<Object[]> chiTietList) {
		super(parent, "Phiếu Trả Thuốc - " + maHoaDon, true);
		...
		// Tiêu đề hóa đơn
		JLabel lblTieuDe = new JLabel("HÓA ĐƠN TRẢ THUỐC");
		...
		JLabel txtMaKH = new JLabel(maKH);
...
//		}
	...
	private void btnInPhieuNhapThuocActionPerformed() {
        ...
        printerJob.setJobName("Hóa Đơn Trả Thuốc");
        ...
        if (printerJob.printDialog()) {
            try {
                printerJob.print();
                JOptionPane.showMessageDialog(this, "In phiếu trả thuốc thành công!", "Thông Báo", JOptionPane.INFORMATION_MESSAGE);
                isPrinting = true;
                dispose();
            } catch (PrinterException ex) {
                JOptionPane.showMessageDialog(this, "Lỗi khi in phiếu trả thuốc: " + ex.getMessage(), "Lỗi In", JOptionPane.ERROR_MESSAGE);
                isPrinting = false;
                ex.printStackTrace();
            }
		} else {
